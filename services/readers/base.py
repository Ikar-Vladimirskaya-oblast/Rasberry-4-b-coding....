    ...
    async def snapshot(self) -> ReaderSnapshot:
        async with self._state_lock:
            return ReaderSnapshot(
                ...
                i2c_address=self.settings.i2c_address,
                i2c_mux_address=self.settings.i2c_mux_address,
                i2c_mux_channel=self.settings.i2c_mux_channel,
                status=self._status,
                mode=self._mode,
                enabled=self.settings.enabled,
                led_enabled=self.settings.led_enabled,
                led_mode=self.settings.led_mode,
                led_gpio_pin=self.settings.led_gpio_pin,
                led_active_high=self.settings.led_active_high,
                led_pixel_count=self.settings.led_pixel_count,
                led_pixel_index=self.settings.led_pixel_index,
